...
- (NSMutableDictionary *)basicLog {
    ...
    //if (self.lt) dic[@"lt"] = self.lt;//dhlu,不需要在这定义，因为每次调用logWithDict都有lt.
    ...
    if (0.0!=self.lnt) dic[@"lnt"] = @(self.lnt);//dhlu,改了一下类型.
    if (0.0!=self.ltt) dic[@"ltt"] = @(self.ltt);//dhlu,改了一下类型.
    if (self.rg) dic[@"rg"] = self.rg;
    if (self.av17) dic[@"av17"] = self.av17;
    if (self.host) dic[@"host"] = self.host;
    //if (self.pt) dic[@"pt"] = self.pt;//dhlu,不再需要。
...
//            wSelf.pingloss = [NSString stringWithFormat:@"%f", _ping.lossPercentage]; //dhlu,不再需要字符串类型.
//            wSelf.pingRtt = [NSString stringWithFormat:@"%li",_ping.averageRetryTime];//dhlu,不再需要字符串类型.
            
            [wSelf logWithDict:@{@"lt": @"pv",
                                 @"prtt": @(_ping.averageRetryTime),//dhlu
                                 @"plss": @(_ping.lossPercentage)//dhlu
                                 }];
        }
    }];
}
